...
impl EntryCache {
    ...
    fn append(&mut self, tag: &str, entries: &[Entry]) {
        ...
        self.append_impl(tag, entries);
        TOTAL_ENTRY_CACHE_COUNT.add((self.cache.len() - current_cache_count) as i64)
    }

    fn append_impl(&mut self, tag: &str, entries: &[Entry]) {
        if entries.is_empty() {
            return;
        }
    ...
    pub fn compact_to(&mut self, idx: u64) {
        let current_cache_count = self.cache.len();
        self.compact_to_impl(idx);
        TOTAL_ENTRY_CACHE_COUNT.add((self.cache.len() - current_cache_count) as i64);
    }

    pub fn compact_to_impl(&mut self, idx: u64) {
